...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scaleVectror = x1 * x2 + y1 * y2;
  const firstVector = Math.sqrt(x1 ** 2 + y1 ** 2);
  const secondVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  const alpha = scaleVectror / (firstVector * secondVector);
 ...
 */
function isPrime(n) {
  const nSrt = Math.sqrt(n);
  for (let i = 2; i <= nSrt; i += 1) {
    if (n % i === 0) {
      return false;
    }
  }
...
function getSumOfDigits(num) {
  const relString = num.toString();
  const relArr = relString.split('');
  let result = 0;
  // eslint-disable-next-line no-return-assign
  relArr.map((el) => (result += Number(el)));
  return result;
}
